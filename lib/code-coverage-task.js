...
const { removePlaceholders } = require('@cypress/code-coverage/common-utils');
const { makeThreadId } = require('./utility');

const threadId = makeThreadId();

const debug = require('debug')(`code-coverage:${threadId}`);
...
const COVERAGE_LOCK_FILE_NAME = join(
  nycReportOptions['temp-dir'],
  'coverage.lock'
);
rmSync(COVERAGE_LOCK_FILE_NAME, { force: true });
...
const tasks = {
   ...
   * @returns {null|Promise<null>} Nothing is returned from this task
   */
  combineCoverage(sentCoverage) {
    const delayExecution = () => {
      // if coverage file is currently locked, wait for a bit, then try again
      const waitForMs = Math.random() * 200 + 100;
      debug(`detected thread-unsafe write access. Wait for ${waitForMs}ms.`);
      return new Promise((resolve) =>
        setTimeout(() => {
          const result = tasks.combineCoverage(sentCoverage);
          if (result === null) {
            // null means task was resolved
            resolve(null);
          } else {
            // not null means task was a promise that has to be resolved afterwards
    ...
    if (existsSync(COVERAGE_LOCK_FILE_NAME)) {
      return delayExecution();
    }

    // create lock file
    writeFileSync(COVERAGE_LOCK_FILE_NAME, threadId);
    // then check again, because it can happen that multiple threads create the file in the same tick
    const fileContents = readFileSync(COVERAGE_LOCK_FILE_NAME, 'utf8');
    if (!fileContents.startsWith(threadId)) {
      debug('Simultaneous access to lock-file detected');
      return delayExecution();
    }
    debug('created lock file %s', COVERAGE_LOCK_FILE_NAME);
    ...
    // delete lockfile
    rmSync(COVERAGE_LOCK_FILE_NAME);
    debug('removed lock file %s', COVERAGE_LOCK_FILE_NAME);
